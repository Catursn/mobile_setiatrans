<?php
class Amplop_model extends CI_model
{
    public function getAmplop($id = null)
    {
        if ($id === null) {
            return $this->db->get('transaksi_amplop')->result_array();
        } else {
            return $this->db->get_where('transaksi_amplop', ['id_amplop' => $id])->result_array();
        }
    }
